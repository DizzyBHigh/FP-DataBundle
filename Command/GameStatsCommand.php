...
class GameStatsCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName( 'fetch_data:gameStats' )
            ->setDescription( 'Pulls GameStats and updates/Inserts to database' )
            ->addOption(
                'season',
                null,
    ...
    protected function execute( InputInterface $input, OutputInterface $output )
    {
        ...
        if ($week == 'none') {
            $gameStats = $fetcher->gameStats( $fullSeason );
            $fetchType = "GameStatsBySeason";
        } else {
            $gameStats = $fetcher->gameStatsByWeek( $fullSeason, $week );
            $fetchType = "GameStatsByWeek";
        }

        $thisCommand = "fetch_data:gameStats --season=".$season." --seasonType=".$seasonType." --week=".$week;
        ...
        if ($gameStats->count() > 0) {
            foreach ($gameStats as $gameStat) {
                if ($week == 'none') {
                    $container->get( 'gameStatsBySeason_persister' )->Persist( $gameStat );
                } else {
                    $container->get( 'gameStatsByWeek_persister' )->Persist( $gameStat );
                }
